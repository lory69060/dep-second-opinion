...
| 13 Phase3.3 | 🔄 凑样本 | 已 6/10（trial PR #1–#6）；中期漏报 40% 预警；见 `trials/log.md` |
| 14 Phase3.4 | ⬜ 修试验暴露缺陷 | `auto_merge_max_bump` 强制抬级；HIGH_RISK 仍发评论且 job 不误红（或软红） |

## Phase 3 原则

- 仍只评论、不改依赖
- 杀伤标准：见试验协议「停做线」
- 步骤 13 进行中；样本未满 10 不判杀。说「继续」优先开 **14（修缺陷）** 再补样本
